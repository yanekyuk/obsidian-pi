...
- While pi is working, choose **Steer · next step** or **Queue · after task** above the composer. Enter (or Send) uses your choice: Steer reaches pi before its next step; Queue waits until pi has finished. Steer is selected by default, and Alt+Enter always queues. Waiting messages are listed above the composer as **Next** and **After**; the ↶ button on the list takes them back into the editor, and Esc stops pi and does the same.
